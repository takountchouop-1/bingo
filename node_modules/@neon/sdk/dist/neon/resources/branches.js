...
var Branches = class {
	...
	async getDefault(projectId, opts) {
		...
		if (!branch) return finalize(err(new NeonClientError("No default branch found for the project.")), shouldThrow);
	...
	async resetFromParent(projectId, branchId, input, opts) {
		...
		if (!parentId) return finalize(err(new NeonClientError("Branch has no parent and cannot be reset.")), shouldThrow);
...
function parseCreateInput(input) {
	...
	if (input.noCompute === true) {
		if ("compute" in input && input.compute !== void 0) return { error: new NeonClientError(NO_COMPUTE_WITH_COMPUTE) };
