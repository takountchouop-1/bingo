...
var Postgres = class {
	...
	async connectionString(params, opts) {
		...
		try {
			...
			const result = cancellation !== void 0 ? err(cancellation) : resolved ?? err(new NeonClientError("The connection-string resolver ended without a result."));
	...
	async #resolve(params, signal) {
		...
		if ((!roleName || !databaseName) && !branchId) {
			...
			if (!branchId) return err(new NeonClientError("Could not determine the default branch; pass branchId."));
		}
		if (!roleName) {
...
function branchRequired(param) {
	return new NeonClientError(`Pass branchId or ${param} to resolve a connection string.`);
}
function ambiguous(kind, count, param) {
	return new NeonClientError(`Expected exactly one ${kind} to auto-select for the connection string; found ${count}. Pass ${param}.`);
}
//#endregion
export { Postgres };
