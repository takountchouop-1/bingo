import { NeonAbortError, NeonOperationError, NeonWaitTimeoutError, toNeonError } from "./errors.js";
...
*/
function readinessEnded(deadline, timeoutMs, outstanding) {
	const source = deadline.source();
	if (source === "caller") return new NeonAbortError("Waiting for operations was aborted by its signal.");
	if (source === "timeout") return new NeonWaitTimeoutError(`Timed out after ${timeoutMs}ms waiting for ${outstanding.length} operation(s) to finish.`, {
		timeoutMs,
...
async function waitForOperations(client, operations, options = {}) {
	...
	try {
		...
		while (pending.length > 0) {
			...
			if (await delay(pollIntervalMs, deadline.signal) === "cancelled") return err(readinessEnded(deadline, timeoutMs, pending) ?? new NeonAbortError("Waiting for operations was aborted by its signal."));
			const stillPending = [];
			for (let i = 0; i < pending.length; i++) {
				...
				const stopped = readinessEnded(deadline, timeoutMs, [...stillPending, ...pending.slice(i)]);
