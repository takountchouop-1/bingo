...
const computeUnitSchema = z.literal(COMPUTE_UNITS, { error: (issue) => `must be a compute size Neon offers: 0.25, 0.5, an integer 1-16, or an even integer 18-56 (got ${JSON.stringify(issue.input)})` });
/**
...
const computeSettingsSchema = z.strictObject({
	...
	if (min === max) return;
	const overMaxMessage = (field, value) => `${field} (${value}) exceeds the autoscaling maximum of ${AUTOSCALING_MAX_CU} CU — sizes above ${AUTOSCALING_MAX_CU} are fixed-size only (set autoscalingLimitMinCu = autoscalingLimitMaxCu)`;
	if (min > AUTOSCALING_MAX_CU) ctx.addIssue({
		code: "custom",
		path: ["autoscalingLimitMinCu"],
		message: overMaxMessage("autoscalingLimitMinCu", min)
	});
	if (max > AUTOSCALING_MAX_CU) ctx.addIssue({
		code: "custom",
		path: ["autoscalingLimitMaxCu"],
		message: overMaxMessage("autoscalingLimitMaxCu", max)
	});
	if (max - min > AUTOSCALING_MAX_RANGE_CU) ctx.addIssue({
		code: "custom",
		path: ["autoscalingLimitMaxCu"],
